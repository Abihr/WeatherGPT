...
export default function WeatherCard({ location, weather, size = "hero", locating }) {
  if (locating) {
    return (
    ...
    return (
      <div className="rounded-xl2 bg-sky-50 px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-ink-800">{weather.condition}</p>
          <p className="text-xs text-ink-400">Humidity {weather.humidity}% · Wind {weather.wind} km/h</p>
        </div>
        <p className="text-xl font-display font-bold text-ink-900">{weather.temp}°</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl3 p-6 sm:p-7 bg-hero-gradient text-white shadow-pop relative overflow-hidden animate-enter">
      <div className="absolute -right-8 -top-10 text-[160px] opacity-15 leading-none select-none">{icon}</div>
      <div className="relative">
        ...
        <p className="text-lg font-medium text-sky-50 mt-2">{weather.condition}</p>
        <p className="text-sm text-sky-100/90">Feels like {weather.feelsLike}°C</p>

        <div className="grid grid-cols-3 gap-3 mt-6">
          <Stat icon={Droplets} label="Humidity" value={`${weather.humidity}%`} />
          <Stat icon={Wind} label="Wind" value={`${weather.wind} km/h`} />
          <Stat icon={CloudRain} label="Rain" value={`${weather.rain}%`} />
        </div>
      </div>
    </div>
  );
